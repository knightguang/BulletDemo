...
@implementation LiveBulletManager
...
- (void)stopLiveBullet
{
    ...
    [self.bulletViews enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        LiveBulletView *view = obj;
        [view stopLiveBulletAnimation];
        view = nil;
    }];
...
- (void)initLiveBulletComments
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (self.bulletComments.count>0) {
            ...
            // 创建一条弹幕view
            [self createLiveBulletView:comment withTrajectory:trajectory];
        }
    }
}

- (void)createLiveBulletView:(NSString *)aComment withTrajectory:(int)trajectory
{
    if (self.isStopAnimation) {
        return;
    }
    
    LiveBulletView *view = [[LiveBulletView alloc] initWithComment:aComment];
    ...
    view.moveStatusBlock = ^(MoveStatus status){
        ...
        switch (status) {
            ...
            case MoveEnter:{
                ...
                if (comment) {
                    [weakSelf createLiveBulletView:comment withTrajectory:trajectory];
                }
                break;
            }
            case MoveEnd:{
                ...
                if ([weakSelf.bulletViews containsObject:weakView]) {
                    [weakView stopLiveBulletAnimation];
